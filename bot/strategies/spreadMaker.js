...
module.exports = {
  ...
  paramSchema: {
    spreadPct:            { label: 'Spread (%)',               type: 'number', min: 0.1, step: 0.1, description: 'Total quoted spread around mid price. Example: 2 means roughly 1% below for bid and 1% above for ask.' },
    orderSize:            { label: 'Order Size (NXS)',         type: 'number', min: 1,   step: 1, description: 'NXS volume for each quote order. Applies to both buy and sell sides.' },
    refreshThresholdPct:  { label: 'Refresh Trigger (%)',      type: 'number', min: 0.1, step: 0.1, description: 'If market mid price moves by this percentage, quotes are canceled and re-posted around the new mid.' },
    maxOrdersPerSide:     { label: 'Tiers Per Side (1-3)',     type: 'number', min: 1,   max: 3, step: 1, description: 'How many quote tiers to place on each side. Higher values add farther quotes at wider spreads.' },
    tierSpreadMultiplier: { label: 'Tier Spread Multiplier',   type: 'number', min: 1.1, step: 0.1, description: 'Multiplier used to widen spread for each additional tier. Larger values push outer tiers farther from mid.' },
  },
  computeTargetOrders,
  needsRebalance,
};
