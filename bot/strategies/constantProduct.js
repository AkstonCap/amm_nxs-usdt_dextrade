...
module.exports = {
  ...
  paramSchema: {
    liquidity:             { label: 'Virtual Liquidity (USDT)', type: 'number', min: 10,  step: 10,  description: 'Controls the total virtual pool depth used by the x·y=k model. Higher values create larger order volumes and stronger liquidity near mid price.' },
    priceRangePct:         { label: 'Price Range (%)',          type: 'number', min: 0.5, step: 0.5, description: 'Sets how far above and below the current mid price orders are placed. Example: 10 means approximately ±10% around mid.' },
    numOrders:             { label: 'Orders Per Side',          type: 'number', min: 1,   max: 20, step: 1, description: 'Defines how many ladder levels are placed on each side (buy and sell). More levels spread liquidity more finely across the range.' },
    rebalanceThresholdPct: { label: 'Rebalance Trigger (%)',    type: 'number', min: 0.1, step: 0.1, description: 'If mid price moves by at least this percentage from the last quote center, existing orders are rebuilt around the new market level.' },
    minOrderSize:          { label: 'Min Order Size (NXS)',     type: 'number', min: 0.1, step: 0.1, description: 'Filters out tiny generated orders. Any computed order smaller than this NXS size is skipped.' },
  },
  computeTargetOrders,
  needsRebalance,
};
